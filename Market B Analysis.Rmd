...
Este algoritmo se utiliza sobre bases de datos transaccionales, y permite encontrar de forma eficiente "conjunto de items frecuentes", los cuales sirven de base para generar reglas de asociación.
...
## Conversion de los datos originales a formato transaccional
...
Dado los objetivos de minería planteados en el anteproyecto, se requiere un soporte mínimo del 3% (creo que necesitamos cambiar este porcentaje), y un mínimo de confianza del 0.85

* Generación de reglas para cada modelo

```{r}
#eclatItemsets <- eclat(TransactionsLoaded, parameter=list(supp=0.001, maxlen=10))
eclatItemsets <- eclat(TransactionsLoaded, parameter=list(supp=0.001, maxlen=10))
...
inspect(sort(EclatRules,by='support',decreasing = TRUE)[1:10])
```

```{r}
#aprioriRules <- apriori(TransactionsLoaded, parameter=list(supp=0.001, conf=0.8, maxlen=10))
tmpAprioriRules <- apriori(TransactionsLoaded, parameter=list(supp=0.001, conf=0.85, maxlen=10))
...
# Ordena items de acuerdo al soporte
inspect(sort(aprioriRules,by='support',decreasing = TRUE)[1:10])
```
...
* Visualización de reglas con gráficos

## Tres mejores reglas según la confianza usando ECLAT

```{r}
# recordar activar arulesViz
eclatTop3Rules<- head(eclatRules,n=3,by='confidence')
plot(eclatTop3Rules,method = 'graph',engine = 'htmlwidget')
```

## Tres mejores reglas según la confianza usando Apriori

```{r}
aprioriTop3Rules<- head(aprioriRules,n=3,by='confidence')
plot(aprioriTop3Rules,method = 'graph',engine = 'htmlwidget')
```
...
Tiempos de ejecución por modelo: 1.15s para el apriori, mientras que para el ECLAT es de 2.3s

* Describir el resultados de ambos modelos

(Pendiente - Brainstorm with Lisi)
